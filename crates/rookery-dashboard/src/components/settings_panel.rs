...
pub fn SettingsPanel(
    ...
    let on_save = move |_| {
        ...
        wasm_bindgen_futures::spawn_local(async move {
            ...
            if let Ok(v) = s.temp.parse::<f64>() { update.insert("temp".into(), serde_json::json!(v)); }
            if let Ok(v) = s.top_p.parse::<f64>() { update.insert("top_p".into(), serde_json::json!(v)); }
            if let Ok(v) = s.top_k.parse::<u64>() { update.insert("top_k".into(), serde_json::json!(v)); }
            if let Ok(v) = s.min_p.parse::<f64>() { update.insert("min_p".into(), serde_json::json!(v)); }
