...
- `rookery-daemon` is a **binary crate** (entrypoint `src/main.rs`) and does not expose `src/lib.rs`.
- `rookery-engine/src/test_utils.rs` is compiled under `#[cfg(test)]`; it is available to `rookery-engine` tests but not importable by other crates' test modules.
- Cross-crate tests (for example in `rookery-daemon`) should use crate-local helpers/mocks rather than importing `rookery-engine` test-only utilities.
- In process/backend tests that write temp scripts and execute them immediately, Linux can hit transient `ETXTBSY` races; current helpers mitigate this by `sync_all`, setting executable perms, and a brief post-write delay.
